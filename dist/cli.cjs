...
async function parseMigration(sql) {
  try {
    const result = await (0, import_libpg_query.parse)(sql);
...
// src/production/context.ts
var { Pool: Pool2 } = esm_default;
async function fetchProductionContext(config, tableNames) {
  if (tableNames.length === 0) {
    return {
      tableStats: /* @__PURE__ */ new Map(),
  ...
  const pool = new Pool2({
    ...
    connectionTimeoutMillis: config.connectTimeout ?? 5e3,
    statement_timeout: config.queryTimeout ?? 1e4,
    max: 1,
    ssl: config.ssl
  });
  try {
...
async function queryTableStats(pool, tableNames) {
...
async function queryAffectedQueries(pool, tableNames) {
...
async function queryActiveConnections(pool, tableNames) {
...
async function findAndLoadConfig(dir) {
  let current = (0, import_node_path.resolve)(dir);
  const root = (0, import_node_path.dirname)(current) === current ? current : void 0;
  for (let i = 0; i < 20; i++) {
    for (const filename of CONFIG_FILES) {
      const filePath = (0, import_node_path.resolve)(current, filename);
      try {
    ...
    try {
      const pkgPath = (0, import_node_path.resolve)(current, "package.json");
...
async function fetchContext(sql, databaseUrl) {
  try {
