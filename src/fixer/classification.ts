...
const TABLE: ReadonlyArray<RuleFixClassification> = [
  { ruleId: 'MP001', fixClass: 'mechanical', reason: 'Insert CONCURRENTLY into CREATE [UNIQUE] INDEX — same index, non-blocking build.' },
  ...
  { ruleId: 'MP004', fixClass: 'mechanical', reason: "Prepend SET lock_timeout = '5s' before the DDL; adds a guard, changes no statement." },
  { ruleId: 'MP005', fixClass: 'mechanical', reason: 'Append NOT VALID to the FOREIGN KEY constraint so creation skips the table scan.' },
  ...
  { ruleId: 'MP009', fixClass: 'mechanical', reason: 'Insert CONCURRENTLY into DROP INDEX, preserving an IF EXISTS clause.' },
  ...
  { ruleId: 'MP012', fixClass: 'mechanical', reason: 'Lift ALTER TYPE ... ADD VALUE out of its BEGIN/COMMIT block when it is the first, last, or only statement in it.' },
  ...
  { ruleId: 'MP020', fixClass: 'mechanical', reason: "Prepend SET statement_timeout = '30s' before long-running DDL." },
  { ruleId: 'MP021', fixClass: 'mechanical', reason: 'Insert CONCURRENTLY into REINDEX TABLE/INDEX/SCHEMA/DATABASE (PG 12+).' },
  ...
  { ruleId: 'MP023', fixClass: 'mechanical', reason: 'Insert IF NOT EXISTS into CREATE TABLE/INDEX, making the statement idempotent.' },
  ...
  { ruleId: 'MP025', fixClass: 'mechanical', reason: 'Lift the CONCURRENTLY statement out of its BEGIN/COMMIT block when it is the first, last, or only statement in it.' },
  ...
  { ruleId: 'MP030', fixClass: 'mechanical', reason: 'Append NOT VALID to the CHECK constraint so creation skips the table scan.' },
  ...
  { ruleId: 'MP033', fixClass: 'mechanical', reason: 'Insert CONCURRENTLY into REFRESH MATERIALIZED VIEW.' },
  ...
  { ruleId: 'MP037', fixClass: 'mechanical', reason: 'VARCHAR(n) → TEXT; PostgreSQL stores both identically, so only the length check is dropped.' },
  { ruleId: 'MP038', fixClass: 'mechanical', reason: 'INT → BIGINT on a PK/FK column, and only inside CREATE TABLE where no rows exist yet.' },
  { ruleId: 'MP039', fixClass: 'mechanical', reason: 'SERIAL → GENERATED BY DEFAULT AS IDENTITY inside CREATE TABLE; identical default behaviour, cleaner sequence ownership.' },
  { ruleId: 'MP040', fixClass: 'mechanical', reason: 'TIMESTAMP → TIMESTAMPTZ.' },
  { ruleId: 'MP041', fixClass: 'mechanical', reason: 'CHAR(n) → TEXT, dropping the blank padding.' },
  { ruleId: 'MP042', fixClass: 'mechanical', reason: 'Write out the name PostgreSQL would have generated (table_col_idx) for plain column-list indexes.' },
  ...
  { ruleId: 'MP046', fixClass: 'mechanical', reason: 'Insert CONCURRENTLY into DETACH PARTITION (PG 14+).' },
  ...
  { ruleId: 'MP074', fixClass: 'mechanical', reason: 'Append DEFERRABLE INITIALLY IMMEDIATE to ALTER TABLE ADD CONSTRAINT ... FOREIGN KEY; same check timing, now deferrable on demand.' },
  ...
  { ruleId: 'MP077', fixClass: 'mechanical', reason: "pglz → lz4 (PG 14+); metadata-only, existing values keep their current compression." },
