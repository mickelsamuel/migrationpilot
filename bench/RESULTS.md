...
Generated **2026-08-12** from 56 labelled migration files.
...
Items marked **Fixed** were open when they were written down and have since been
closed; the wording of the finding is left as it was recorded so the fix can be
read against the complaint. Everything else is still open.

### MigrationPilot: the MP084-MP112 batch cost precision, not recall — fixed
...
### MigrationPilot: a leading comment switches off transaction analysis — fixed
...
### MigrationPilot: MP058 argues against the handbook — fixed
...
### MigrationPilot: MP070 and MP097 cannot both be satisfied — fixed
...
### MigrationPilot: MP097 treats every unique index as constraint-backed — fixed
...
### MigrationPilot: MP090 warns about the trigger that prevents the outage — fixed
...
```console
$ migrationpilot analyze bench/corpus/safe/s09-expand-contract-rename-step1.sql --quiet
```

### MigrationPilot: MP003 reports the opposite of the manual — fixed
...
### MigrationPilot: the comment bug is not hypothetical — it eats a whole agent file — fixed
...
No tool changed its findings because of a leading comment.
...
| `agent-flavored/a02-order-status-enum.sql` | dangerous | `enum-add-value-in-transaction`, `unbatched-backfill` | ⚠️ 1/2 | ⚠️ other | ⚠️ 1/2 |
| `agent-flavored/a03-multi-tenant-schema.sql` | dangerous | `multi-table-ddl-transaction`, `fk-without-not-valid`, `non-concurrent-index` | ✅ all | ⚠️ other | ⚠️ other |
...
| `agent-flavored/a08-agent-got-it-right.sql` | safe | — | ✅ clean | ✅ clean | ✅ clean |
...
| `safe/s02-not-null-via-valid-check.sql` | safe | — | ✅ clean | ✅ clean | ⚠️ 2 flags |
...
| `safe/s05-foreign-key-not-valid-then-validate.sql` | safe | — | ✅ clean | ✅ clean | ✅ clean |
...
| `safe/s11-check-constraint-not-valid.sql` | safe | — | ✅ clean | ✅ clean | ✅ clean |
