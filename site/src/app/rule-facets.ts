...
export const facetsByRuleId: Record<string, RuleFacets> = {
  ...
  MP043: {
    ...
    triggersOn:
      'Two branches. First, a `CreateDomainStmt` whose `constraints[].Constraint.contype` is `CONSTR_CHECK`. Second, an `AlterDomainStmt` whose `subtype` equals the single character `T`. The second branch does not do what its message claims: libpg-query emits `subtype: "C"` for `ALTER DOMAIN ... ADD CONSTRAINT` and `subtype: "T"` for `ALTER DOMAIN ... SET DEFAULT` / `DROP DEFAULT`, both verified against the bundled parser. So the branch actually fires on default changes and reports them as an ADD CONSTRAINT scan.',
    doesNotTriggerOn:
      '`ALTER DOMAIN ... ADD CONSTRAINT ... CHECK` — the one statement the rule is named for — because of the `subtype !== "T"` mismatch above. `CREATE DOMAIN` carrying only `NOT NULL` (`CONSTR_NOTNULL`) or only a `DEFAULT`, since neither is `CONSTR_CHECK`. `ALTER DOMAIN ... DROP CONSTRAINT` (`subtype "X"`) and `SET NOT NULL` / `DROP NOT NULL` (`O` / `N`). Ordinary table-level `CHECK` constraints, which are MP030.',
    lockDetail:
      'The hazard the rule is written about is real: `ALTER DOMAIN ... ADD CONSTRAINT` takes ACCESS EXCLUSIVE on every table that has a column of that domain and validates the predicate against all of them, so a domain used by twenty tables produces twenty simultaneous blocking full scans. But the statement the rule actually catches is `CREATE DOMAIN`, which touches no table at all and takes no table lock, plus `ALTER DOMAIN ... SET DEFAULT`, which is a catalog-only change.',
    assumptions:
      'With no catalog access the rule cannot know how many columns use the domain, so it cannot distinguish a brand-new domain nothing references from one wired through half the schema — and it warns identically either way. The `subtype` mix-up above means it is simultaneously a false positive on `SET DEFAULT` and a false negative on the exact statement that causes the multi-table scan. Nothing in the code reads `ctx.cluster`, `ctx.tableStats`, or any other production field, so `--database-url` does not change its behaviour.',
  },
  MP044: {
    operation: 'column',
  ...
  MP048: {
    ...
    triggersOn:
      'An `AlterTableStmt` command with subtype `AT_ColumnDefault`, where `JSON.stringify(cmd.AlterTableCmd.def).toLowerCase()` contains any of the substrings `now`, `random`, `gen_random_uuid`, `uuid_generate_v4`, `clock_timestamp`, `statement_timestamp`, `timeofday`, `txid_current`, `nextval`. This is a raw substring search over the serialised parse node, not a walk looking for `FuncCall.funcname` — unlike `containsVolatileCall` in `catalog-helpers.ts`, which does it properly.',
    doesNotTriggerOn:
      '`ALTER COLUMN ... DROP DEFAULT`, which is also `AT_ColumnDefault` but arrives with no `def`, so the serialised string is `{}` and nothing matches. `SET DEFAULT` with a plain constant that contains none of those substrings. `ADD COLUMN ... DEFAULT now()`, which is `AT_AddColumn` and belongs to MP003 and MP015. Any non-`AlterTableStmt` node.',
    lockDetail:
    ...
    assumptions:
      'The substring match over serialised JSON produces real false positives. `ALTER TABLE t ALTER COLUMN c SET DEFAULT \'nowhere\'` fires, verified against the bundled parser, because the literal `nowhere` contains `now`. So does any default mentioning a function or identifier with `random` or `now` inside it. In the other direction, a user-defined volatile function with an unrelated name is not on the list and passes silently. The rule also cannot tell whether a backfill is actually wanted — setting a default purely for new rows is a legitimate thing to do, and gets flagged anyway.',
  },
  MP049: {
    operation: 'partition',
    impacts: ['deploy-incompat'],
  ...
  MP052: {
    ...
    triggersOn:
      'An `AlterTableStmt` with a command of subtype `AT_DropColumn` or `AT_AlterColumnType`. The loop returns on the first such command, and the two produce differently worded messages — the drop case also ships a `pg_depend` query in `safeAlternative` that names the specific table and column.',
    doesNotTriggerOn:
      '`RENAME COLUMN`, despite being the second item in the rule description and in the doc. The rule opens with `if (!("AlterTableStmt" in stmt)) return null;` and `ALTER TABLE ... RENAME COLUMN` parses as a `RenameStmt`, so the `RenameStmt` block further down is unreachable. It is dead twice over: it also compares `renameType` against the number `7`, while the bundled parser emits the string `"OBJECT_COLUMN"` — both confirmed by probing the parser. `ADD COLUMN`, `SET NOT NULL`, and every other subtype are outside the rule, and there is no test covering the rename path.',
    lockDetail:
