/*
 * The first paint of the hero analyzer.
 *
 * This is not a mock. It is the verbatim JSON report the real engine produced
 * for DEFAULT_SQL below, captured so the page can render a true result before
 * the 1.15 MB WebAssembly parser has loaded. Once the engine is in memory the
 * widget analyses live and this object is never used again.
 *
 * Any long dash inside a violation message or safe alternative is the engine
 * talking, quoted exactly. Do not edit those strings here; fix the rule.
 *
 * Regenerate after any rule change:
 *   node dist/cli.cjs analyze <file holding DEFAULT_SQL> --offline --format json
 */
