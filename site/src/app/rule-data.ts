...
export interface RuleInfo {
  ...
  /** True when the rule reads live catalog state and is silent without `--database-url`. */
...
CREATE MATERIALIZED VIEW daily_revenue AS
  ...
  },
  {
    ...
    badExample: `-- The UNIQUE constraint owns users_email_key, here or in an earlier migration:
...
// Catalog metadata
//
// Grouping and a short label for every rule, so no page has to keep its own copy
// of the rule list. An inline copy is how the homepage ended up showing 18 rule
// names that no longer matched the engine — and those strings are exactly what
// users put in .migrationpilotrc.yml, where a wrong name fails silently.
//
// Names, severities, auto-fix flags and requiresDatabaseUrl in `rules` above are
// the engine's, verified against `migrationpilot list-rules --json`.
// ─────────────────────────────────────────────────────────────────────────────
...
const CATEGORY_BY_ID: Record<string, RuleCategory> = {
  MP001: 'Lock safety',
  MP002: 'Constraints & keys',
  MP003: 'Lock safety',
  MP004: 'Lock safety',
  MP005: 'Constraints & keys',
  MP006: 'Lock safety',
  MP007: 'Lock safety',
  MP008: 'Lock safety',
  MP009: 'Lock safety',
  MP010: 'Data safety',
  MP011: 'Backfills & DML',
  MP012: 'Types & schema style',
  MP013: 'Production context',
  MP014: 'Production context',
  MP015: 'Types & schema style',
  MP016: 'Constraints & keys',
  MP017: 'Data safety',
  MP018: 'Constraints & keys',
  MP019: 'Production context',
  MP020: 'Lock safety',
  MP021: 'Lock safety',
  MP022: 'Data safety',
  MP023: 'Types & schema style',
  MP024: 'Data safety',
  MP025: 'Lock safety',
  MP026: 'Data safety',
  MP027: 'Constraints & keys',
  MP028: 'Data safety',
  MP029: 'Data safety',
  MP030: 'Constraints & keys',
  MP031: 'Constraints & keys',
  MP032: 'Lock safety',
  MP033: 'Lock safety',
  MP034: 'Data safety',
  MP035: 'Data safety',
  MP036: 'Data safety',
  MP037: 'Types & schema style',
  MP038: 'Types & schema style',
  MP039: 'Types & schema style',
  MP040: 'Types & schema style',
  MP041: 'Types & schema style',
  MP042: 'Types & schema style',
  MP043: 'Constraints & keys',
  MP044: 'Data safety',
  MP045: 'Constraints & keys',
  MP046: 'Partitioning',
  MP047: 'Lock safety',
  MP048: 'Types & schema style',
  MP049: 'Partitioning',
  MP050: 'Extensions',
  MP051: 'Extensions',
  MP052: 'Data safety',
  MP053: 'Lock safety',
  MP054: 'Types & schema style',
  MP055: 'Data safety',
  MP056: 'Types & schema style',
  MP057: 'Privileges & RLS',
  MP058: 'Lock safety',
  MP059: 'Backfills & DML',
  MP060: 'Data safety',
  MP061: 'Types & schema style',
  MP062: 'Types & schema style',
  MP063: 'Lock safety',
  MP064: 'Data safety',
  MP065: 'Lock safety',
  MP066: 'Types & schema style',
  MP067: 'Backfills & DML',
  MP068: 'Types & schema style',
  MP069: 'Lock safety',
  MP070: 'Lock safety',
  MP071: 'Data safety',
  MP072: 'Partitioning',
  MP073: 'Privileges & RLS',
  MP074: 'Constraints & keys',
  MP075: 'Types & schema style',
  MP076: 'Lock safety',
  MP077: 'Types & schema style',
  MP078: 'Types & schema style',
  MP079: 'Privileges & RLS',
  MP080: 'Backfills & DML',
  MP081: 'Constraints & keys',
  MP082: 'Constraints & keys',
  MP083: 'Constraints & keys',
  MP084: 'Constraints & keys',
  MP085: 'Privileges & RLS',
  MP086: 'Constraints & keys',
  MP087: 'Constraints & keys',
  MP088: 'Backfills & DML',
  MP089: 'Lock safety',
  MP090: 'Lock safety',
  MP091: 'Privileges & RLS',
  MP092: 'Partitioning',
  MP093: 'Partitioning',
  MP094: 'Partitioning',
  MP095: 'Lock safety',
  ...
  MP097: 'Data safety',
  MP098: 'Data safety',
  MP099: 'Privileges & RLS',
  MP100: 'Production context',
  MP101: 'Production context',
  MP102: 'Production context',
  MP103: 'Production context',
  MP104: 'Production context',
  MP105: 'Extensions',
  MP106: 'Extensions',
  MP107: 'Extensions',
  MP108: 'Extensions',
...
/** Short label for compact listings, where the full description is too long. */
const SHORT_DESC_BY_ID: Record<string, string> = {
  MP001: 'CREATE INDEX without CONCURRENTLY',
  MP002: 'SET NOT NULL without CHECK pattern',
  MP003: 'ADD COLUMN with volatile DEFAULT',
  MP004: 'DDL without SET lock_timeout',
  MP005: 'FK without NOT VALID',
  MP006: 'VACUUM FULL blocks everything',
  MP007: 'ALTER COLUMN TYPE rewrites table',
  MP008: 'Multiple DDL in one transaction',
  MP009: 'DROP INDEX without CONCURRENTLY',
  MP010: 'RENAME COLUMN breaks queries',
  MP011: 'UPDATE without WHERE',
  MP012: 'ADD VALUE inside transaction',
  MP013: 'DDL on high-traffic table',
  MP014: 'Lock on 1M+ row table',
  MP015: 'SERIAL → use IDENTITY',
  MP016: 'FK without index',
  MP017: 'DROP COLUMN risks',
  MP018: 'SET NOT NULL scan',
  MP019: 'ACCESS EXCLUSIVE + many connections',
  MP020: 'DDL without timeout',
  MP021: 'REINDEX without CONCURRENTLY',
  MP022: 'CASCADE drops dependents',
  MP023: 'Non-idempotent CREATE',
  MP024: 'DROP TYPE destroys enum',
  MP025: 'CONCURRENTLY inside transaction',
  MP026: 'DROP TABLE permanently',
  MP027: 'UNIQUE without USING INDEX',
  MP028: 'RENAME TABLE breaks refs',
  MP029: 'DROP NOT NULL risks',
  MP030: 'CHECK without NOT VALID',
  MP031: 'EXCLUSION constraint',
  MP032: 'CLUSTER rewrites table',
  MP033: 'REFRESH without CONCURRENTLY',
  MP034: 'DROP DATABASE in migration',
  MP035: 'DROP SCHEMA permanently',
  MP036: 'TRUNCATE CASCADE across tables',
  MP037: 'VARCHAR → use TEXT',
  MP038: 'INT PK → use BIGINT',
  MP039: 'SERIAL → use IDENTITY',
  MP040: 'TIMESTAMP → use TIMESTAMPTZ',
  MP041: 'CHAR(n) wastes space',
  MP042: 'Unnamed index',
  MP043: 'Domain constraint risk',
  MP044: 'Narrowing column type',
  MP045: 'Table without PK',
  MP046: 'DETACH PARTITION without CONCURRENTLY',
  MP047: 'SET LOGGED/UNLOGGED rewrites table',
  MP048: 'Volatile SET DEFAULT',
  MP049: 'PK missing partition key column',
  MP050: 'IVFFlat → use HNSW',
  MP051: 'Geometry column without GIST',
  MP052: 'DROP/ALTER breaks views/functions',
  MP053: 'BEGIN without COMMIT',
  MP054: 'ADD VALUE in transaction',
  MP055: 'DROP PK breaks replication',
  MP056: 'Missing GIN index for JSONB',
  MP057: 'RLS enabled without policy',
  MP058: 'Multiple ALTER TABLE on same table',
  MP059: 'Sequence not reset after import',
  MP060: 'RENAME VALUE holds ACCESS EXCLUSIVE',
  MP061: 'Column order wastes alignment',
  MP062: 'GENERATED STORED rewrites table',
  MP063: 'DDL inside DO $$ block',
  MP064: 'DISABLE TRIGGER breaks replication',
  MP065: 'LOCK TABLE blocks queries',
  MP066: 'Autovacuum disabled on table',
  MP067: 'Backfill without batching',
  MP068: 'Integer PK nearing overflow',
  MP069: 'FK locks both parent and child',
  MP070: 'CONCURRENTLY can leave invalid index',
  MP071: 'Rename column used by views/indexes',
  MP072: 'Partition default scans all rows',
  MP073: 'SUPERUSER role in migration',
  MP074: 'FK without DEFERRABLE',
  MP075: 'TOAST table bloat risk',
  MP076: 'Retry loop consuming XIDs',
  MP077: 'Use LZ4 over PGLZ for TOAST',
  MP078: 'Extension version not pinned',
  MP079: 'RLS policy missing operations',
  MP080: 'Data changes in schema migration',
  MP081: 'CHECK workaround no longer needed on PG18+',
  MP082: 'NOT ENFORCED constraint does not enforce',
  MP083: 'FK on non-deterministic collation',
  MP084: 'ADD COLUMN NOT NULL without DEFAULT',
  MP085: 'GRANT to PUBLIC or GRANT ALL',
  MP086: 'FK without explicit ON DELETE',
  MP087: 'CHECK constraint calling now()/random()',
  MP088: 'Backfill without a following ANALYZE',
  MP089: 'COLLATE change rewrites and reindexes',
  MP090: 'FOR EACH ROW trigger on every write',
  MP091: 'GRANT/REVOKE mixed into DDL migration',
  MP092: 'Index on partitioned parent locks all',
  MP093: 'DEFAULT partition absorbs missing rows',
  MP094: 'ATTACH PARTITION without matching CHECK',
  MP095: 'SET TABLESPACE copies the whole relation',
  MP096: 'CREATE MATERIALIZED VIEW WITH DATA',
  MP097: 'Dropping a constraint-backed index',
  MP098: 'SET SCHEMA breaks qualified references',
  MP099: 'SECURITY DEFINER without pinned search_path',
  MP100: 'Index duplicates an existing prefix',
  MP101: 'New index on a write-hot table',
  MP102: 'Rewrite needs a second full copy',
  MP103: 'WAL-heavy work with replicas connected',
  MP104: 'Index build measured in hours',
  MP105: 'Hypertable DDL fans out to chunks',
  MP106: 'Retention DELETE instead of drop_chunks()',
  MP107: 'ALTER propagates to every Citus shard',
  MP108: 'Manual DDL on a pg_partman parent',
  MP109: 'pgvector index without build parameters',
  MP110: 'DDL locks parent and every partition',
  MP111: 'TimescaleDB rejects this on columnstore',
...
/** Every rule, enriched for listing. Throws at import if metadata is missing. */
...
export const ruleCategories: { title: RuleCategory; rules: RuleCatalogEntry[] }[] =
  ruleCategoryOrder.map((title) => ({
    title,
    rules: ruleCatalog.filter((rule) => rule.category === title),
  }));

/**
 * Rules that read live catalog state — table sizes, write traffic, replication,
 * installed extensions — and stay silent without `--database-url`. Free like every
 * other rule; they simply have nothing to read without a connection.
 */
export const productionContextRuleIds: readonly string[] = rules
  .filter((rule) => rule.requiresDatabaseUrl)
  .map((rule) => rule.id);

/** The rules `--fix` can rewrite in place. */
export const autoFixableRuleIds: readonly string[] = rules
