        ...
        <article className="mp-container pt-16 md:pt-20 pb-20">
          <div className="max-w-3xl">
            ...
            <div className="prose prose-invert max-w-none">
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Why Regular CREATE INDEX Is Dangerous</h2>

              <p className="text-muted leading-relaxed mb-4">
                A regular <code className="rounded bg-raised px-1.5 py-0.5 font-mono text-[0.9em] text-fg">CREATE INDEX</code> acquires a SHARE lock on the table.
              ...
              <CodeBlock code={`-- DANGEROUS: Blocks all writes for the entire build duration
CREATE INDEX idx_orders_customer_id ON orders (customer_id);

-- On a 50M row table, this might take 3-5 minutes
-- Every INSERT, UPDATE, DELETE queues behind this lock
-- Connection pools fill up, application errors cascade`} language="sql" />

              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">How CREATE INDEX CONCURRENTLY Works</h2>
              ...
              <ol className="list-decimal list-inside text-muted space-y-4 mb-6 ml-4">
                <li>
                  <strong className="text-fg">Phase 1 — Catalog entry:</strong> PostgreSQL creates the index
                ...
                <li>
                  <strong className="text-fg">Phase 2 — First table scan:</strong> PostgreSQL scans the entire table
                ...
                <li>
                  <strong className="text-fg">Phase 3 — Second table scan:</strong> PostgreSQL does a second pass
              ...
              <CodeBlock code={`-- SAFE: Does not block reads or writes
CREATE INDEX CONCURRENTLY idx_orders_customer_id ON orders (customer_id);

-- Takes longer than regular CREATE INDEX (roughly 2-3x)
-- but your application continues running normally`} language="sql" />

              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">The Transaction Restriction</h2>
              ...
              <CodeBlock code={`-- This fails immediately
BEGIN;
...
CREATE INDEX CONCURRENTLY idx_email ON users (email);`} language="sql" />
                ...
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="border-b border-line">
                      ...
                      <th className="text-left py-3 px-4 text-muted font-semibold">How to Disable Transaction</th>
                    </tr>
                  </thead>
                  <tbody className="text-muted">
                    <tr className="border-b border-line-soft">
                      <td className="py-3 px-4 font-semibold text-faint">Django</td>
                      <td className="py-3 px-4 font-mono text-sm">atomic = False on the Migration class</td>
                    </tr>
                    <tr className="border-b border-line-soft">
                      <td className="py-3 px-4 font-semibold text-faint">Rails</td>
                      <td className="py-3 px-4 font-mono text-sm">disable_ddl_transaction!</td>
                    </tr>
                    <tr className="border-b border-line-soft">
                      <td className="py-3 px-4 font-semibold text-faint">Flyway</td>
                      <td className="py-3 px-4 font-mono text-sm">executeInTransaction=false (in SQL comment)</td>
                    </tr>
                    <tr className="border-b border-line-soft">
                      <td className="py-3 px-4 font-semibold text-faint">Alembic</td>
                      <td className="py-3 px-4 font-mono text-sm">{`op.execute() with connection.execution_options(isolation_level="AUTOCOMMIT")`}</td>
                    </tr>
                    <tr className="border-b border-line-soft">
                      <td className="py-3 px-4 font-semibold text-faint">Knex</td>
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Failure Modes and Recovery</h2>

              <p className="text-muted leading-relaxed mb-4">
                CREATE INDEX CONCURRENTLY can fail partway through. Unlike a regular CREATE INDEX (which rolls
                back cleanly on failure), a failed CONCURRENTLY operation leaves behind an <strong className="text-fg">invalid index</strong>.
              </p>
              ...
              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">Detecting invalid indexes</h3>

              <CodeBlock code={`-- Find all invalid indexes in the database
SELECT
  schemaname,
  tablename,
  indexname,
...
WHERE indexname IN (
  ...
  WHERE NOT indisvalid
              ...
              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">Recovering from a failed build</h3>

              <p className="text-muted leading-relaxed mb-4">
                You have two options when a concurrent index build fails:
              </p>

              <CodeBlock code={`-- Option 1: Drop and rebuild
...
REINDEX INDEX CONCURRENTLY idx_orders_customer_id;`} language="sql" />
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">REINDEX CONCURRENTLY (PostgreSQL 12+)</h2>
              ...
              <CodeBlock code={`-- Rebuild a single index without blocking writes
...
REINDEX INDEX idx_orders_customer_id;  -- Don't do this in production`} language="sql" />

              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Performance Considerations</h2>
              ...
              <CodeBlock code={`-- Increase maintenance_work_mem for faster index builds
...
RESET max_parallel_maintenance_workers;`} language="sql" />

              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">DROP INDEX CONCURRENTLY</h2>
              ...
              <CodeBlock code={`-- DANGEROUS: Blocks all reads and writes
...
-- inside a transaction block`} language="sql" />

              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Retry Pattern for CI/CD</h2>
...
exit 1`} />
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Unique Index Considerations</h2>
              ...
              <CodeBlock code={`-- Check for duplicates before creating a unique index
SELECT email, count(*)
FROM users
GROUP BY email
HAVING count(*) > 1
...
CREATE UNIQUE INDEX CONCURRENTLY idx_users_email ON users (email);`} language="sql" />

              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Summary: The Rules</h2>
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Automate the Check</h2>
              ...
              <CommandBlock command={`npx migrationpilot analyze migrations/004_add_indexes.sql --fix`} />
            </div>
          </div>
        </article>
      </main>
