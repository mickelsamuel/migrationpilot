        ...
        <article className="mp-container pt-16 md:pt-20 pb-20">
          <div className="max-w-3xl">
            ...
            <div className="prose prose-invert max-w-none">
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Mistake #1: CREATE INDEX Without CONCURRENTLY</h2>

              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">The dangerous migration</h3>

              <CodeBlock code={`-- This looks harmless
CREATE INDEX idx_orders_customer_id ON orders (customer_id);`} language="sql" />

              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">What happens in production</h3>

              <p className="text-muted leading-relaxed mb-4">
                A regular <code className="rounded bg-raised px-1.5 py-0.5 font-mono text-[0.9em] text-fg">CREATE INDEX</code> acquires
                a <strong className="text-fg">SHARE lock</strong> on the table. This lock blocks all INSERT, UPDATE,
              ...
              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">The safe pattern</h3>

              <CodeBlock code={`-- Add CONCURRENTLY — builds the index without blocking writes
CREATE INDEX CONCURRENTLY idx_orders_customer_id ON orders (customer_id);`} language="sql" />

              <p className="text-muted leading-relaxed mb-4">
                <code className="rounded bg-raised px-1.5 py-0.5 font-mono text-[0.9em] text-fg">CREATE INDEX CONCURRENTLY</code> acquires
                only a <strong className="text-fg">SHARE UPDATE EXCLUSIVE lock</strong>, which allows reads and writes
              ...
              <p className="text-muted leading-relaxed mb-6">
                <strong className="text-warn">Important caveat:</strong> CONCURRENTLY cannot run inside a transaction.
              ...
              <CodeBlock code={`-- Flyway: disable transaction for this migration
...
-- Then retry the CREATE INDEX CONCURRENTLY`} language="sql" />

              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Mistake #2: ALTER TABLE SET NOT NULL Without the CHECK Pattern</h2>

              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">The dangerous migration</h3>

              <CodeBlock code={`-- Adding NOT NULL to an existing column
ALTER TABLE users ALTER COLUMN email SET NOT NULL;`} language="sql" />

              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">What happens in production</h3>

              <p className="text-muted leading-relaxed mb-4">
                <code className="rounded bg-raised px-1.5 py-0.5 font-mono text-[0.9em] text-fg">SET NOT NULL</code> acquires
                an <strong className="text-fg">ACCESS EXCLUSIVE lock</strong> &mdash; the most restrictive lock
                in PostgreSQL. This lock blocks <em>everything</em>: reads, writes, even other
                <code className="rounded bg-raised px-1.5 py-0.5 font-mono text-[0.9em] text-fg">SELECT</code> queries.
              </p>
              ...
              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">The safe pattern (PostgreSQL 12+)</h3>

              <CodeBlock code={`-- Step 1: Add a CHECK constraint with NOT VALID
...
ALTER TABLE users DROP CONSTRAINT users_email_not_null;`} language="sql" />
              ...
              <p className="text-muted leading-relaxed mb-6">
                ...
                validated CHECK constraint that guarantees no NULLs exist, the <code className="rounded bg-raised px-1.5 py-0.5 font-mono text-[0.9em] text-fg">SET NOT NULL</code> becomes
                a metadata-only operation that completes instantly.
              </p>

              <div className="bg-accent-soft border border-accent/35 rounded-lg p-4 mb-6">
                <p className="text-muted text-sm">
                  <strong className="text-accent">PostgreSQL 18 update:</strong> As of PostgreSQL 18 (September 2025),
                  you can use <code className="rounded bg-raised px-1.5 py-0.5 font-mono text-[0.9em] text-fg">ALTER TABLE ... ADD COLUMN ... NOT NULL NOT VALID</code> directly.
                  This simplifies the pattern for new columns, though the CHECK pattern is still needed for
                  existing columns.
                </p>
              </div>

              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Mistake #3: ALTER COLUMN TYPE (Silent Table Rewrite)</h2>

              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">The dangerous migration</h3>

              <CodeBlock code={`-- Changing a column type
ALTER TABLE events ALTER COLUMN payload TYPE jsonb USING payload::jsonb;

-- Or seemingly innocent changes like:
ALTER TABLE users ALTER COLUMN name TYPE varchar(500);  -- was varchar(255)`} language="sql" />

              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">What happens in production</h3>

              <p className="text-muted leading-relaxed mb-4">
                Most column type changes trigger a <strong className="text-fg">full table rewrite</strong>.
              ...
              <p className="text-muted leading-relaxed mb-4">
                ...
                <code className="rounded bg-raised px-1.5 py-0.5 font-mono text-[0.9em] text-fg">varchar(255)</code> to
                <code className="rounded bg-raised px-1.5 py-0.5 font-mono text-[0.9em] text-fg">varchar(500)</code> seems like
                it should be a metadata change, but PostgreSQL rewrites the entire table.
              </p>

              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">Safe alternatives</h3>

              <p className="text-muted leading-relaxed mb-4">
                There is no universal safe pattern for column type changes. The approach depends on the specific change:
              </p>

              <CodeBlock code={`-- varchar(255) → text: SAFE, no rewrite (just removes the length limit)
...
-- 3. Swap columns
-- 4. Drop old column`} language="sql" />
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Mistake #4: Running Migrations Without lock_timeout</h2>

              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">The dangerous migration</h3>

              <CodeBlock code={`-- No lock_timeout set — will wait indefinitely for the lock
ALTER TABLE orders ADD COLUMN discount_pct numeric(5,2);`} language="sql" />
              ...
              <p className="text-muted leading-relaxed mb-4">
                ...
                your DDL statement will <strong className="text-fg">wait indefinitely</strong> for the lock.
              </p>

              <p className="text-muted leading-relaxed mb-4">
                While your DDL statement is waiting, it goes into the lock queue. Here&apos;s the critical part:
                <strong className="text-fg"> every subsequent query on that table also gets queued behind your
              ...
              <CodeBlock code={`-- Always set lock_timeout before DDL operations
SET lock_timeout = '5s';
...
-- Reset for safety
RESET lock_timeout;`} language="sql" />

              <p className="text-muted leading-relaxed mb-4">
                With <code className="rounded bg-raised px-1.5 py-0.5 font-mono text-[0.9em] text-fg">lock_timeout</code> set,
              ...
              <CodeBlock code={`-- Retry wrapper pattern (pseudocode)
for attempt in 1..5:
  BEGIN;
...
-- If all 5 attempts fail, alert and investigate`} language="sql" />

              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Mistake #5: Adding a Foreign Key Without NOT VALID</h2>

              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">The dangerous migration</h3>

              <CodeBlock code={`-- Adding a foreign key constraint
ALTER TABLE orders
  ADD CONSTRAINT orders_customer_fk
  FOREIGN KEY (customer_id) REFERENCES customers (id);`} language="sql" />

              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">What happens in production</h3>

              <p className="text-muted leading-relaxed mb-4">
                Adding a foreign key constraint acquires locks on <strong className="text-fg">both tables</strong> &mdash;
              ...
              <CodeBlock code={`-- Step 1: Add the constraint with NOT VALID
-- This only checks new/modified rows, no full table scan
...
ALTER TABLE orders VALIDATE CONSTRAINT orders_customer_fk;`} language="sql" />
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Common Thread: The Lock Queue Problem</h2>
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Preventing These Mistakes</h2>
              ...
              <CodeBlock code={`# Check all migration files for safety issues
...
#       SET lock_timeout = '5s';`} />
              ...
              <ul className="list-disc list-inside text-muted space-y-2 mb-6 ml-4">
                <li><a href="https://github.com/sbdchd/squawk" className="text-accent hover:text-accent-hover">Squawk</a> &mdash; 40 rules, Rust, GitHub Action</li>
                <li><a href="https://github.com/ankane/strong_migrations" className="text-accent hover:text-accent-hover">strong_migrations</a> &mdash; Rails only, excellent error messages</li>
                <li><a href="https://github.com/mickelsamuel/migrationpilot" className="text-accent hover:text-accent-hover">MigrationPilot</a> &mdash; 112 rules, lock classification, auto-fix, GitHub Action</li>
              </ul>
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Quick Reference</h2>

              <div className="overflow-x-auto mb-8">
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="border-b border-line">
                      <th className="text-left py-3 px-4 text-muted font-semibold">Mistake</th>
                      <th className="text-left py-3 px-4 text-muted font-semibold">Lock Acquired</th>
                      <th className="text-left py-3 px-4 text-muted font-semibold">Safe Pattern</th>
                    </tr>
                  </thead>
                  <tbody className="text-muted">
                    <tr className="border-b border-line-soft">
                      <td className="py-3 px-4">CREATE INDEX (no CONCURRENTLY)</td>
                      <td className="py-3 px-4 text-danger">SHARE</td>
                    ...
                    <tr className="border-b border-line-soft">
                      ...
                      <td className="py-3 px-4 text-warn">Any (queues indefinitely)</td>
                      <td className="py-3 px-4">SET lock_timeout = &apos;5s&apos;</td>
                    </tr>
                    <tr className="border-b border-line-soft">
                      <td className="py-3 px-4">FK without NOT VALID</td>
                      <td className="py-3 px-4 text-danger">SHARE ROW EXCLUSIVE (both tables)</td>
