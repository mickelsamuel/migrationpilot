        ...
        <article className="mp-container pt-16 md:pt-20 pb-20">
          <div className="max-w-3xl">
            ...
            <div className="prose prose-invert max-w-none">
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">The Problem with SET NOT NULL</h2>

              <p className="text-muted leading-relaxed mb-4">
                When you run <code className="rounded bg-raised px-1.5 py-0.5 font-mono text-[0.9em] text-fg">ALTER TABLE ... ALTER COLUMN ... SET NOT NULL</code>,
              ...
              <CodeBlock code={`-- DANGEROUS: Full table scan under ACCESS EXCLUSIVE
...
-- During step 2, no query can read or write the table`} language="sql" />
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">The Safe Pattern: CHECK + VALIDATE</h2>
              ...
              <CodeBlock code={`-- Step 1: Add the constraint as NOT VALID
...
ALTER TABLE users VALIDATE CONSTRAINT chk_email_not_null;`} language="sql" />
                ...
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="border-b border-line">
                      <th className="text-left py-3 px-4 text-muted font-semibold">Aspect</th>
                      <th className="text-left py-3 px-4 text-muted font-semibold">SET NOT NULL</th>
                      <th className="text-left py-3 px-4 text-muted font-semibold">CHECK + VALIDATE</th>
                    </tr>
                  </thead>
                  <tbody className="text-muted">
                    <tr className="border-b border-line-soft">
                      ...
                      <td className="py-3 px-4 text-ok">ACCESS EXCLUSIVE (instant)</td>
                    </tr>
                    <tr className="border-b border-line-soft">
                      <td className="py-3 px-4">Lock type (validate)</td>
                      <td className="py-3 px-4 text-danger">Same lock, same operation</td>
                      <td className="py-3 px-4 text-ok">SHARE UPDATE EXCLUSIVE</td>
                    </tr>
                    <tr className="border-b border-line-soft">
                      <td className="py-3 px-4">Blocks reads?</td>
                      <td className="py-3 px-4 text-danger">Yes (during scan)</td>
                      <td className="py-3 px-4 text-ok">No</td>
                    </tr>
                    <tr className="border-b border-line-soft">
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">PostgreSQL 12+: Converting CHECK to NOT NULL</h2>

              <p className="text-muted leading-relaxed mb-4">
                Starting with PostgreSQL 12, if a validated CHECK constraint of the form{' '}
                <code className="rounded bg-raised px-1.5 py-0.5 font-mono text-[0.9em] text-fg">CHECK (column IS NOT NULL)</code> exists,
                PostgreSQL recognizes that the column already has a not-null guarantee. In this case,{' '}
                <code className="rounded bg-raised px-1.5 py-0.5 font-mono text-[0.9em] text-fg">SET NOT NULL</code> skips the table scan
                and completes instantly:
              </p>

              <CodeBlock code={`-- Full safe pattern for PG 12+:
...
ALTER TABLE users DROP CONSTRAINT chk_email_not_null;`} language="sql" />
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">PostgreSQL 11 and Earlier</h2>
              ...
              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">Option A: Keep the CHECK constraint (recommended)</h3>

              <CodeBlock code={`-- Just use the CHECK constraint as your not-null guarantee
...
-- existing and future rows. No need for SET NOT NULL.`} language="sql" />

              <p className="text-muted leading-relaxed mb-6">
                The CHECK constraint provides the same guarantee as SET NOT NULL. The only difference is
                cosmetic: <code className="rounded bg-raised px-1.5 py-0.5 font-mono text-[0.9em] text-fg">\d users</code> will not show
              ...
              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">Option B: Accept the brief lock (small tables only)</h3>

              <CodeBlock code={`-- If the table is small enough (< 100K rows), the scan
...
RESET lock_timeout;`} language="sql" />
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Handling Existing NULL Values</h2>
              ...
              <CodeBlock code={`-- Step 0: Backfill NULL values (before adding the constraint)
...
ALTER TABLE users VALIDATE CONSTRAINT chk_email_not_null;`} language="sql" />
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Common Mistakes</h2>

              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">1. Forgetting NOT VALID</h3>

              <CodeBlock code={`-- BAD: Without NOT VALID, ADD CONSTRAINT scans the table
...
ALTER TABLE users ADD CONSTRAINT chk_email_not_null
  CHECK (email IS NOT NULL) NOT VALID;`} language="sql" />

              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">2. Running SET NOT NULL on PG 11 after CHECK validation</h3>

              <CodeBlock code={`-- BAD (on PG 11): SET NOT NULL still scans the table
...
-- On PG 12+: SET NOT NULL is instant (skips scan)`} language="sql" />

              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">3. Not setting lock_timeout</h3>

              <CodeBlock code={`-- BAD: Both steps should have lock_timeout
...
ALTER TABLE users VALIDATE CONSTRAINT chk_email_not_null;`} language="sql" />
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">The Complete Pattern</h2>

              <p className="text-muted leading-relaxed mb-4">
                Here is the full, production-safe pattern for adding NOT NULL to an existing column:
              </p>

              <CodeBlock code={`-- Migration file: 005_add_email_not_null.sql
...
RESET lock_timeout;`} language="sql" />
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">Automate This Check</h2>
              ...
              <CommandBlock command={`npx migrationpilot analyze migrations/005_add_email_not_null.sql`} />
            </div>
          </div>
        </article>
      </main>
