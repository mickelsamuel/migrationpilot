        ...
        <article className="mp-container pt-16 md:pt-20 pb-20">
          <div className="max-w-3xl">
            ...
            <div className="prose prose-invert max-w-none">
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">1. Always Set lock_timeout</h2>
              ...
              <CodeBlock code={`-- Set a 5-second lock timeout
...
RESET lock_timeout;`} language="sql" />
              ...
              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">The retry wrapper pattern</h3>

              <CodeBlock code={`-- Retry wrapper: try up to 5 times with increasing delays
DO $$
DECLARE
  retries INT := 0;
  max_retries INT := 5;
...
END $$;`} language="sql" />
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">2. Use statement_timeout for Long-Running DDL</h2>
              ...
              <CodeBlock code={`-- Prevent DDL from running longer than expected
SET lock_timeout = '5s';
...
RESET lock_timeout;
RESET statement_timeout;`} language="sql" />

              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">3. The Expand-Contract Pattern</h2>
              ...
              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">Example: Renaming a column</h3>
              ...
              <CodeBlock code={`-- DEPLOY 1: Expand — add the new column alongside the old one
...
UPDATE users SET full_name = name WHERE full_name IS NULL;`} language="sql" />

              <CodeBlock code={`-- DEPLOY 2: Migrate — update application to use "full_name"
-- All reads and writes now use the new column name
-- Deploy this code change and verify in production`} />

              <CodeBlock code={`-- DEPLOY 3: Contract — remove the old column and trigger
DROP TRIGGER trg_sync_names ON users;
DROP FUNCTION sync_name_columns();
ALTER TABLE users DROP COLUMN name;`} language="sql" />

              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">Example: Changing a column type</h3>

              <CodeBlock code={`-- Changing INT to BIGINT directly acquires ACCESS EXCLUSIVE
...
-- Optionally: ALTER TABLE orders RENAME COLUMN amount_v2 TO amount;`} language="sql" />

              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">4. Batched Backfills</h2>
              ...
              <CodeBlock code={`-- BAD: Single update that touches all 50M rows
...
END $$;`} language="sql" />
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">5. Safe Constraint Addition</h2>

              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">NOT NULL constraints</h3>
              ...
              <CodeBlock code={`-- BAD: Scans entire table under ACCESS EXCLUSIVE
ALTER TABLE users ALTER COLUMN email SET NOT NULL;
...
ALTER TABLE users ALTER COLUMN email SET NOT NULL;
ALTER TABLE users DROP CONSTRAINT chk_email_not_null;`} language="sql" />

              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">Foreign key constraints</h3>

              <CodeBlock code={`-- BAD: Validates all existing rows under SHARE ROW EXCLUSIVE
...
ALTER TABLE orders VALIDATE CONSTRAINT fk_user;`} language="sql" />

              <h3 className="text-xl font-semibold mt-8 mb-3 text-fg">Unique constraints</h3>

              <CodeBlock code={`-- BAD: Creates an index + validates under ACCESS EXCLUSIVE
ALTER TABLE users ADD CONSTRAINT uniq_email UNIQUE (email);

-- GOOD: Build the index concurrently, then attach it
CREATE UNIQUE INDEX CONCURRENTLY idx_users_email ON users (email);
ALTER TABLE users ADD CONSTRAINT uniq_email UNIQUE USING INDEX idx_users_email;`} language="sql" />

              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">6. One DDL Per Transaction</h2>
              ...
              <CodeBlock code={`-- BAD: Three DDL statements in one transaction
...
-- locks released
ALTER TABLE users ADD COLUMN city TEXT;
-- locks released`} language="sql" />

              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">7. Test Migrations Against Production Data</h2>
              ...
              <ul className="list-disc list-inside text-muted space-y-2 mb-6 ml-4">
                ...
                <li>Use <code className="rounded bg-raised px-1.5 py-0.5 font-mono text-[0.9em] text-fg">EXPLAIN</code> on any data migration queries to check for sequential scans</li>
                <li>Monitor <code className="rounded bg-raised px-1.5 py-0.5 font-mono text-[0.9em] text-fg">pg_stat_activity</code> and <code className="rounded bg-raised px-1.5 py-0.5 font-mono text-[0.9em] text-fg">pg_locks</code> during staging runs</li>
                <li>Run static analysis on migration files in CI to catch common issues early</li>
              </ul>

              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">8. Deployment Ordering</h2>
                ...
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="border-b border-line">
                      <th className="text-left py-3 px-4 text-muted font-semibold">Change Type</th>
                      <th className="text-left py-3 px-4 text-muted font-semibold">Deploy Order</th>
                      <th className="text-left py-3 px-4 text-muted font-semibold">Why</th>
                    </tr>
                  </thead>
              ...
              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">9. Monitor During Migrations</h2>

              <p className="text-muted leading-relaxed mb-4">
                Always monitor these metrics during migration execution:
              </p>

              <CodeBlock code={`-- Active locks on the table being migrated
...
SELECT client_addr, state,
       ...
       replay_lag
FROM pg_stat_replication;`} language="sql" />

              <h2 className="text-2xl font-bold mt-12 mb-4 text-fg">10. Enforce Best Practices in CI</h2>
              ...
              <p className="text-muted leading-relaxed mb-6">
                ...
                checks all of these patterns (and 106 more) as a CLI, GitHub Action, or Node.js library.
              ...
              <CodeBlock code={`# Add to .github/workflows/migration-check.yml
