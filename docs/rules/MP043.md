...
-- If you must add one to an existing domain, add it NOT VALID first. The
-- constraint applies to new writes immediately and nothing is scanned until
-- you choose to run VALIDATE CONSTRAINT:
ALTER DOMAIN email_address ADD CONSTRAINT valid_email
  CHECK (length(VALUE) <= 255) NOT VALID;
```

## What It Does Not Flag
`ALTER DOMAIN ... ADD CONSTRAINT ... NOT VALID`, which is the safe form above.
`SET DEFAULT`, `DROP DEFAULT`, `SET NOT NULL`, `DROP NOT NULL` and
`DROP CONSTRAINT` on a domain -- none of them validate existing values.
`CREATE DOMAIN` carrying only a `DEFAULT` or a bare `NOT NULL`. Ordinary
table-level `CHECK` constraints belong to MP030.
